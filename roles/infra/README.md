...
A role that pre-deploys Front-end VMs directly in Libvirt after installing `opennebula-node-kvm` and configuring Libvirt in target hypervisor.
...
**NOTE**: Use `--skip-tags libvirt` when pre-installed Libvirt software is available in `infra_hostname`.
