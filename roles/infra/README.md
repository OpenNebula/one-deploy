...
| `vcpu_shares`                        | `str`  | `200`                                    |                     | The total CPU share for the domain.                               |
|                                      |        |                                          |                     |                                                                   |
...
| `dpdk_socket_path`                   | `str`  | `/var/run/one/vhost-socks/frontend.sock` |                     | Path for existing socket when using OVS with DPDK.                |
|                                      |        |                                          |                     |                                                                   |
| `infra_hostname`                     | `str`  |                                          | `n1a1`              | Defines on which HV machine the Front-end VM should be deployed.  |
| `infra_xml_variant`                  | `str`  | `default`                                |                     | Defined which domain XML variant will be used in Libvirt.         |
|                                      |        |                                          |                     |                                                                   |
...
| `context.START_SCRIPT_BASE64`        | `str`  |                                          |                     | Start script (base64-encoded automatically).                      |
...
-----------------
    ...
    infra:
      vars:
        os_image_url: http://10.2.11.1/images/ubuntu2404-7.4.0-0-20260714.qcow2
        os_image_size: 20G
    ...
    infra:
      vars:
        ...
        os_image_size: 20G
        memory_KiB: 2097152 # 2 GiB
        infra_xml_variant: openvswitch_dpdk
      hosts:
        u1q20: { ansible_host: 10.3.10.20, dpdk_socket_path: /var/run/one/vhost-socks/u1q40.sock }
          ...
          iface:
            ...
            dpdk-p1:
              set:
                ...
                - options:vhost-server-path: "{{ dpdk_socket_path }}"
          br:
            ovsbr1:
              ports: [dpdk-p0, dpdk-p1]
              set:
